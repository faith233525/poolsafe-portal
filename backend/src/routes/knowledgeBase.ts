    ...
    try {
      ...
      if (search) {
        where.OR = [
          ...
          {
            searchKeywords: { contains: search as string, mode: "insensitive" },
          },
        ];
      }
...
knowledgeBaseRouter.post(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const newRating =
        currentRating === 0 ? rating : (currentRating + rating) / 2;
...
knowledgeBaseRouter.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to fetch knowledge base statistics" });
    }
  }
);
