...
export const apiKeyAuth = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!apiKey) {
    logHelpers.logSecurity('API key missing', {
      ip: req.ip,
  ...
  if (apiKey !== expectedApiKey) {
    logHelpers.logSecurity('Invalid API key attempt', {
      ip: req.ip,
      userAgent: req.get('User-Agent'),
      url: req.url,
      method: req.method,
  ...
  logHelpers.logSecurity('API key authenticated', {
    ip: req.ip,
    url: req.url,
    method: req.method
  }, 'low');
...
export const createRateLimit = (
  ...
  return rateLimit({
    ...
    handler: (req: Request, res: Response) => {
      logHelpers.logSecurity('Rate limit exceeded', {
        ip: req.ip,
        userAgent: req.get('User-Agent'),
        url: req.url,
        method: req.method,
...
export const createSlowDown = (windowMs: number, delayAfter: number, maxDelayMs: number = 5000) => {
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    logHelpers.logSecurity('Slow down triggered', {
      ip: req.ip,
      userAgent: req.get('User-Agent'),
      url: req.url,
      method: req.method,
...
export const ipWhitelist = (allowedIPs: string[] = []) => {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    const isAllowed = allowedIPs.some(allowedIP => 
      allIPs.some(ip => ip?.includes(allowedIP))
    );

    if (!isAllowed) {
      logHelpers.logSecurity('IP not whitelisted', {
        clientIP,
        forwarded,
        realIP,
        userAgent: req.get('User-Agent'),
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (contentLength) {
      ...
      if (sizeInBytes > maxSizeInBytes) {
        logHelpers.logSecurity('Request size exceeded', {
          ip: req.ip,
...
export const validateRequest = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (suspiciousPattern) {
    logHelpers.logSecurity('Suspicious URL pattern detected', {
      ip: req.ip,
      url: req.url,
      decodedUrl: url,
...
export const honeypot = (req: Request, res: Response, next: NextFunction) => {
  ...
  const hasHoneypotField = honeypotFields.some(field => 
    req.body && req.body[field] !== undefined
  );

  if (hasHoneypotField) {
    logHelpers.logSecurity('Honeypot triggered', {
      ip: req.ip,
      userAgent: req.get('User-Agent'),
      url: req.url,
      method: req.method,
