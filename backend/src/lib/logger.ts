...
export const logHelpers = {
  ...
  logAuth: (event: string, userId?: string, ip?: string, userAgent?: string, success: boolean = true) => {
    const logData = {
      event,
      userId,
      ip,
  ...
  // Log API requests
  logRequest: (method: string, url: string, userId?: string, responseTime?: number, statusCode?: number) => {
    const logData = {
      method,
      url,
      userId,
  ...
  // Log business events
  logBusiness: (event: string, entityType: string, entityId: string, userId?: string, details?: any) => {
    const logData = {
      event,
      entityType,
      entityId,
  ...
  logDatabase: (operation: string, table: string, recordId?: string, userId?: string, executionTime?: number) => {
    const logData = {
      operation,
      table,
      recordId,
