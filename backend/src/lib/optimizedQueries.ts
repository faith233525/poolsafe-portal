...
export class OptimizedQueries {
  ...
  async getTicketsPaginated(
    ...
    try {
      ...
      logHelpers.logDatabase("SELECT_PAGINATED", "ticket", undefined, partnerId, executionTime);
  ...
  async getPartnerWithStats(partnerId: string) {
    ...
    try {
      ...
      logHelpers.logDatabase("SELECT_WITH_STATS", "partner", partnerId, undefined, executionTime);
  ...
  async createMultipleNotifications(
    ...
    try {
      ...
      logHelpers.logDatabase("BATCH_INSERT", "notification", undefined, undefined, executionTime);
  ...
  async searchTickets(
    ...
    try {
      ...
      logHelpers.logDatabase("SEARCH", "ticket", undefined, partnerId, executionTime);
  ...
  async bulkUpdateTicketStatus(ticketIds: string[], status: string, partnerId: string) {
    const startTime = Date.now();
    ...
    try {
      ...
      const executionTime = Date.now() - startTime;
      logger.error("Error in bulkUpdateTicketStatus", {
        error,
        executionTime,
        ticketIds,
        partnerId,
  ...
  async getDashboardSummary(partnerId: string) {
    ...
    try {
      const [ticketStats, recentTickets, serviceRecordCount, upcomingEvents] = await Promise.all([
        // Ticket statistics
      ...
      logHelpers.logDatabase("DASHBOARD_SUMMARY", "multiple", partnerId, undefined, executionTime);
